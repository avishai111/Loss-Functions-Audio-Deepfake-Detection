...
class AOCloss(nn.Module):
    ...
    def forward(self, embeddings, labels=None):
        bonafide_embeddings = embeddings[labels == 1] # assuming 1 is bonafide
        fake_embeddings = embeddings[labels == 0] # assuming 0 is spoof
...
if __name__ == "__main__":
    ...
    # Generate random embeddings and corresponding labels (1: bonafide, 0: spoof)
    embeddings = torch.randn(100, embedding_dim)
    labels = torch.cat([torch.ones(10, dtype=torch.int), torch.zeros(90, dtype=torch.int)])
